...
rc=$?
t_eq "packing with no signing certificate fails" \
	"$([ "$rc" -ne 0 ] && echo yes)" yes
...
rc=$?
t_eq "packing with a key that is not there fails" \
	"$([ "$rc" -ne 0 ] && echo yes)" yes

...
The first half of that comparison has now been taken. The `image-identity` job
ran for the first time and is green: the container lane and the native arm64
lane produce the same manifest — the partition table, the image description's
identity fields, and the package set, as `scripts/image-manifest.sh` records
them. Two things that measurement does not say. It compares manifests rather
than images, so anything the manifest does not cover can still differ; and both
lanes are CI's, so what is shown is that the container lane agrees with the
native one, not yet that a developer's workstation does.

Deferred because what remains is hardening the comparison to full-image
equality now that the two lanes are observed to agree, rather than a change to
the build.
...
That precondition has now been met: the `image-identity` job is green on its
first run, so the container lane has a record of producing what the native one
does at manifest level. What remains is a decision rather than a discovery —
whether to pin podman (third-party repository or a build from source) before or
after moving the release-shaped build onto the container, which would make the
question moot for the lane that matters.
